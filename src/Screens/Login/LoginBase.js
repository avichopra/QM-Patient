...
export default class LoginBase extends Component {
	...
	onSubmit = () => {
		...
		if (this.checkAllField()) {
			...
			let fields={"patientId":{"userId":{"_id":1,"fullname": 1,"email": 1,"emergencycontactnumber":1,"contactNo":1,"picture":1}},"pickedPatient":1,"vehicleNo":1,"patientAddress":1,"driverAddress":1,"hospitalName":1,"hospitalAddress":1,"hospitalNo":1,"hospitalLocation":{"lat":1,"long":1},"patientLocation":{"lat":1,"long":1},"driverLocation":{"lat":1,"long":1},"driverId":{"userId":{"deleted":1,"role":1,"createdAt":1,"fullname":1,"email":1,"contactNo":1,"emailVerificationCode":1,"phoneVerified":1,"online":1,"deviceId":1,"picture":1}}}
			callApi('post', 'v1/auth/login', data)
				.then((response) => {
					...
					Promise.all([callApi(
						...
						{ perPage: 1, fields:fields,filter: { status:"Progress",patientId: response.data.user.id } },
						headers
					),callApi("post","v1/daffo/Requests/getOwn",{perPage:1,filter:{status:"Pending",requestedBy:response.data.user.id}},headers)]).then(response=>{
